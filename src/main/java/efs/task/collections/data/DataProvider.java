...
public class DataProvider {
    ...
    public List<Town> getTownsList() {
        ...
        for (String listTowns : Data.baseTownsArray) {
            String[] townData = listTown.split(DATA_SEPARATOR);
            List<String> startingHeroes = new ArrayList<>();
            startingHeroes.add(townData[1].trim());
            startingHeroes.add(townData[2].trim());
            listTowns.add(new Town(townData[0], startingHeroes));
        }
        return listTowns;
    }
    ...
    public List<Town> getDLCTownsList() {
        ...
        for (String listDlcTowns : Data.dlcTownsArray) {
            String[] townData = listDlcTowns.split(DATA_SEPARATOR);
            List<String> startingHeroes = new ArrayList<>();
            startingHeroes.add(townData[1].trim());
            startingHeroes.add(townData[2].trim());
    ...
    public Set<Hero> getHeroesSet() {
        ...
        for (String hero : Data.baseCharactersArray) {
            ...
            heroesSet.add(new Hero(heroData[0].trim(), heroData[1].trim()));
        }
        return heroesSet;
    }
    ...
    public Set<Hero> getDLCHeroesSet() {
        ...
        for (String hero : Data.dlcCharactersArray) {
            String[] heroData = hero.split(DATA_SEPARATOR);
           heroesDlcSet.add(new Hero(heroData[0].trim(), heroData[1].trim()));
        }
        return heroesDlcSet;
    }
}
